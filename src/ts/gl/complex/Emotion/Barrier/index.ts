...
export class Barrier extends THREE.Mesh {
	...
	private initGPUComputationController( size: THREE.Vector2 ) {
		...
		let posUni = ORE.UniformsLib.mergeUniforms( {
			dataPos: { value: null },
			dataVel: { value: null },
		}, this.commonUniforms );
		...
		let velUni = ORE.UniformsLib.mergeUniforms( {
			dataPos: { value: null },
			dataVel: { value: null },
		}, this.commonUniforms );
	...
	private createPoints( size: THREE.Vector2 ) {
		...
		this.pointUni = ORE.UniformsLib.mergeUniforms( {
			posTex: {
				value: null
			},
			velTex: {
		...
		let velViewer = new THREE.Mesh( new THREE.PlaneGeometry( vSize, vSize ), new THREE.ShaderMaterial( {
			...
			uniforms: ORE.UniformsLib.mergeUniforms( { selector: { value: 0 } }, this.pointUni )
		} ) );
		...
		let posViewer = new THREE.Mesh( new THREE.PlaneGeometry( vSize, vSize ), new THREE.ShaderMaterial( {
			fragmentShader: viewerFrag,
			vertexShader: viewerVert,
			uniforms: ORE.UniformsLib.mergeUniforms( { selector: { value: 1 } }, this.pointUni )
		} ) );
